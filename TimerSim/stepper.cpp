...
#define AWAIT_TIMED_PULSE(DIR) while (PULSE_##DIR##_TICK_COUNT > HAL_timer_get_count(PULSE_TIMER_NUM) - start_pulse_count) { } //TODO
...
void Stepper::set_directions() {
    SET_STEP_DIR(X_AXIS); // A

    SET_STEP_DIR(Y_AXIS); // B

    SET_STEP_DIR(Z_AXIS); // C
   ...
   */
    ...
    int32_t Stepper::_eval_bezier_curve(const uint32_t curr_step) {
...
void Stepper::isr() {
  ...
  do {
    ...
    const uint32_t interval = _MIN(
      nextMainISR
      , uint32_t(0xFFFFFFFF)                    // Come back in a very long time
    );
